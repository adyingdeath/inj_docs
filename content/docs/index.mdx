...
for (let i = 0; i < 10; i++) {
  ...
  $(`particle minecraft:end_rod ~${x.toFixed(2)} ~ ~${z.toFixed(2)} 0 0 0 0 1`);
}
```

This will create a circle of end rods around the execution position.

Why do I use `$` here? Actually, `$` doesn't have any special function, it just means to run a Minecraft command at its position. The below two are the same effects:
...
What is special about `$` is that you can inject JavaScript code into the Minecraft command it runs. This expands the possibilities of what you can do with INJ. You will learn more about the usage of `$` in the [$](/docs/api/$) section later.
